...
* tmp
** 发展性计算障碍
发展性计算障碍（developmental dyscalculia, DD）是一种影响数字认知和算术技能学习的发育型疾病。
其通常表现为难以处理和加工数字，计算准确性差等。这些困难并不是智力发展问题、
感官系统损伤、精神或神经疾病、学习资源缺乏所导致的[27]。目前，全球大约5%-7%的人群患有计算障碍[28, 29]。
一些研究认为，计算障碍是处理符号数字的能力受损导致的[30]，由于该损伤**很难随时间改变**，该差异可以被用于DD的诊断[31]；
另一些研究表明，处理或表达数量的能力受损（即ANS功能异常[32]）与DD有很强的相关性。
根据上述理论，DD可以被认为是数字认知缺失的重要体现，是研究基本数字认知的绝佳切入点，
